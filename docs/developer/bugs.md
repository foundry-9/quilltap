...
feature-spec work
**Status**: Bugs **1–64** are **fixed in v4**; Bug **65** is **open**. Bug 65
(filed 2026-08-13, noticed while verifying bug 64 on a fresh instance): the
version guard has been silently inert since 2026-08-12. `version-guard.ts`
reaches into `migrations/lib/database-utils` with a synchronous `require()`,
and that module became an **async module** in Turbopack's graph when the bug 58
fix added a static `instance-lock` import to it — a sync `require()` of an async
module returns an exports object that is never populated, so every call throws
`isSQLiteBackend is not a function` into a catch that allows startup anyway.
`highest_app_version` is never stored and `minServerVersion` never reaches
`.dbkey`, so an older binary will open a newer database without complaint. Fix
spec (`await import`, plus surfacing the failure and testing the *effect*) is in
the bug file. Bug 64
