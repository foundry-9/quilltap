...
**A note on the wardrobe.** The whole of the character's wardrobe now keeps to a single folder, `Wardrobe/`, which holds one Markdown file per item — garments and grand ensembles alike. The frontmatter carries the `title`, the `types` list (one or more of `top`, `bottom`, `footwear`, `accessories`), an optional `imagePrompt` (the Portrait Cue: a short, literal phrase whispered to the portraitist and the Lantern *in place of* the title, since the prose description below is written for human eyes and never reaches the easel), an optional `appropriateness` tag, the `default` flag, and the timestamps; the body of the file is the freeform description, written however you please.
