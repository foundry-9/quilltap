...
export function getUserDataBasePath(userId: string): string {
  ...
  logger.debug('Resolved user data base path', {
    userId,
    userPath,
  });
  return userPath;
...
export function getUserDataPath(userId: string, subPath: string): string {
  ...
  logger.debug('Resolved user data path', {
    userId,
    subPath,
    fullPath,
  });
...
export async function ensureUserDataDir(userId: string): Promise<void> {
  ...
  try {
    ...
    if (stats.isDirectory()) {
      logger.debug('User data directory already exists', { userId, userPath });
      return;
    }
  } catch (error: any) {
...
export async function userDataDirExists(userId: string): Promise<boolean> {
  ...
  try {
    ...
    logger.debug('User data directory existence check', {
      userId,
      userPath,
      exists,
    });
    ...
    if (error.code === 'ENOENT') {
      logger.debug('User data directory does not exist', {
        userId,
        userPath,
      });
      return false;
...
export async function needsDataMigration(): Promise<boolean> {
  try {
    ...
    if (usersDirExists) {
      logger.debug('New user data structure already exists, no migration needed');
      return false;
    }
    ...
    logger.info('Data migration check', {
      usersDirExists,
      legacyFileExists,
      needsMigration,
    });
...
export function getLegacyDataPath(subPath: string): string {
  ...
  logger.debug('Resolved legacy data path', {
    subPath,
    legacyPath,
  });
  return legacyPath;
